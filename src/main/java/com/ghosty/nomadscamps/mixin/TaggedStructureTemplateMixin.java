...
public abstract class TaggedStructureTemplateMixin implements TaggedStructureTemplate {
    @Shadow private final List<StructureTemplate.PalettedBlockInfoList> blockInfoLists = Lists.newArrayList();
    @Shadow private final List<StructureTemplate.StructureEntityInfo> entities = Lists.newArrayList();
    @Shadow private Vec3i size;
    @Shadow private static void categorize(StructureTemplate.StructureBlockInfo blockInfo, List<StructureTemplate.StructureBlockInfo> fullBlocks, List<StructureTemplate.StructureBlockInfo> blocksWithNbt, List<StructureTemplate.StructureBlockInfo> otherBlocks) {}
    @Shadow private static List<StructureTemplate.StructureBlockInfo> combineSorted(List<StructureTemplate.StructureBlockInfo> fullBlocks, List<StructureTemplate.StructureBlockInfo> blocksWithNbt, List<StructureTemplate.StructureBlockInfo> otherBlocks) { return null; }
    @Shadow private void addEntitiesFromWorld(World world, BlockPos firstCorner, BlockPos secondCorner) {}
            ...
            for(BlockPos blockPos4 : BlockPos.iterate(blockPos2, blockPos3)) {
                ...
                if (!blockState.isIn(ignoredBlocks)) {
                    ...
                    if (blockEntity != null) {
                        ...
                        structureBlockInfo = new StructureTemplate.StructureBlockInfo(blockPos5, blockState, (NbtCompound)null);
                    }
